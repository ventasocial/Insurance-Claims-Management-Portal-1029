    ...
    <Layout title="Panel del Cliente">
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Mis Reclamos</h2>
            <p className="text-gray-600 text-sm sm:text-base">Gestiona y da seguimiento a tus reclamos de seguro</p>
          </div>
          <button
          ...
          {filteredClaims.length === 0 ? (
            <div className="p-8 sm:p-12 text-center">
              <div className="w-16 sm:w-24 h-16 sm:h-24 mx-auto mb-4 bg-gray-100 rounded-full flex items-center justify-center">
                <SafeIcon icon={FiCalendar} className="w-8 sm:w-12 h-8 sm:h-12 text-gray-400" />
              </div>
              <h3 className="text-lg font-medium text-gray-900 mb-2">No hay reclamos</h3>
              <p className="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base">No tienes reclamos que coincidan con los filtros seleccionados</p>
              <button
                onClick={() => navigate('/crear-reclamo')}
                className="bg-primary text-white px-4 sm:px-6 py-2 rounded-lg hover:bg-primary-dark transition-colors text-sm sm:text-base"
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-4 sm:p-6 pb-4">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">REC-{claimCode}</p>
                          </div>
                          <div className={`px-2 sm:px-3 py-1 text-xs font-medium rounded-full border flex-shrink-0 ml-2 ${getStatusColor(claim.status)}`}>
                            {claim.status}
                          </div>
                        </div>
                        ...
                        <div className="space-y-2 sm:space-y-3">
                          <div className="flex items-center space-x-2 text-xs sm:text-sm">
                            <SafeIcon icon={FiFileText} className="w-3 sm:w-4 h-3 sm:h-4 text-gray-500 flex-shrink-0" />
                            <span className="text-gray-600">Tipo:</span>
                            <span className="font-medium text-gray-900 truncate">{getClaimTypeDisplay(claim)}</span>
                          </div>
                          <div className="flex items-center space-x-2 text-xs sm:text-sm">
                            <SafeIcon icon={FiUser} className="w-3 sm:w-4 h-3 sm:h-4 text-gray-500 flex-shrink-0" />
                            <span className="text-gray-600">Asegurado:</span>
                            <span className="font-medium text-gray-900 truncate">{claim.customerName}</span>
                          </div>
                          <div className="flex items-center space-x-2 text-xs sm:text-sm">
                            <SafeIcon icon={FiClock} className="w-3 sm:w-4 h-3 sm:h-4 text-gray-500 flex-shrink-0" />
                            <span className="text-gray-600">Fecha:</span>
                            <span className="font-medium text-gray-900">{claim.date}</span>
                          </div>
                          <div className="flex items-center space-x-2 text-xs sm:text-sm">
                            <SafeIcon icon={FiMapPin} className="w-3 sm:w-4 h-3 sm:h-4 text-gray-500 flex-shrink-0" />
                            <span className="text-gray-600">Servicio:</span>
                            <span className="font-medium text-gray-900 truncate">{claim.serviceType}</span>
                          </div>
                          <div className="flex items-center space-x-2 text-xs sm:text-sm">
                            <SafeIcon icon={FiFileText} className="w-3 sm:w-4 h-3 sm:h-4 text-gray-500 flex-shrink-0" />
                            <span className="text-gray-600">Póliza:</span>
                            <span className="font-medium text-gray-900 truncate">{claim.policyNumber}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border-t border-gray-100">
                        <h4 className="text-xs sm:text-sm font-medium text-gray-700 mb-2 sm:mb-3">Personas involucradas</h4>
                        <div className="flex flex-wrap gap-1 sm:gap-2">
                          {involvedPeople.slice(0, 3).map((person, index) => (
                            <div key={index} className="flex items-center bg-white rounded-full px-2 sm:px-3 py-1 sm:py-2 text-xs border border-gray-200 max-w-full">
                              <img
                                src={person.avatar}
                                alt={person.name}
                                className="w-3 sm:w-5 h-3 sm:h-5 rounded-full object-cover mr-1 sm:mr-2 flex-shrink-0"
                              />
                              <div className="flex flex-col min-w-0">
                                <span className="font-medium text-gray-900 truncate max-w-[80px] sm:max-w-[100px]" title={person.name}>
                                  {person.name}
                                </span>
                          ...
                          {involvedPeople.length > 3 && (
                            <div className="flex items-center bg-white rounded-full px-2 sm:px-3 py-1 sm:py-2 text-xs border border-gray-200">
                              <span className="text-gray-500">+{involvedPeople.length - 3}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      {/* Botones de acción */}
                      <div className="p-4 sm:p-6 pt-3 sm:pt-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between space-y-2 sm:space-y-0">
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
                          ...
                          {claim.claimType === 'Reembolso' && claim.claimInitialType === 'Inicial' && (
                            <button
                              ...
                              className="flex items-center justify-center space-x-2 text-blue-600 hover:text-blue-800 transition-colors px-3 py-2 border border-blue-300 rounded-lg hover:bg-blue-50 text-xs font-medium"
                            >
                          ...
                          <button
                            ...
                            className="flex items-center justify-center space-x-2 text-primary hover:text-primary-dark transition-colors px-3 py-2 border border-primary rounded-lg hover:bg-primary-light hover:bg-opacity-10 text-xs font-medium"
                          >
                        ...
                        {claim.status !== 'Archivado' && (
                          <button
                            ...
                            className="flex items-center justify-center space-x-1 text-gray-400 hover:text-gray-600 transition-colors px-2 py-1 border border-gray-200 rounded-lg hover:bg-gray-100 text-xs mt-2 sm:mt-0"
                          >
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg max-w-md w-full mx-4">
              <div className="flex items-center space-x-2 mb-4">
                <SafeIcon icon={FiAlertTriangle} className="w-5 sm:w-6 h-5 sm:h-6 text-orange-500" />
                <h3 className="text-base sm:text-lg font-medium text-gray-900">Confirmar Archivo</h3>
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3">
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 transition-colors text-sm"
                >
                  Archivar Reclamo
                </button>
              </div>
