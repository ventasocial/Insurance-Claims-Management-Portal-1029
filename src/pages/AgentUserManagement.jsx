...
import { mockUsers, mockClientGroups, mockClaims, mockAgentGroupAssignments } from '../data/mockData';
...
const AgentUserManagement = () => {
  ...
  const [newUser, setNewUser] = useState({
    ...
    maternalLastName: '',
    emails: [],
    role: 'client', // Siempre será cliente para agentes
    status: 'active',
    groupId: '',
    avatar: '',
    whatsapp: ''
  ...
  // Obtener grupos asignados al agente actual
  useEffect(() => {
    if (user && user.role === 'staff') {
      const assignments = mockAgentGroupAssignments.filter(assignment => assignment.agentId === user.id);
      const assignedGroupIds = assignments.map(assignment => assignment.groupId);
      const assignedGroups = mockClientGroups.filter(group => assignedGroupIds.includes(group.id));
      setAgentGroups(assignedGroups);
    }
  }, [user]);

  // Filtrar usuarios que pertenecen a los grupos del agente
  useEffect(() => {
    if (agentGroups.length > 0) {
      const agentGroupIds = agentGroups.map(group => group.id);
      const agentUsers = mockUsers.filter(user => 
        user.role === 'client' && agentGroupIds.includes(user.groupId)
      );
      setUsers(agentUsers);
    }
  }, [agentGroups]);
  ...
  const handleAddUser = () => {
    ...
    setNewUser({
      ...
      maternalLastName: '',
      emails: [],
      role: 'client', // Siempre será cliente
      status: 'active',
      groupId: agentGroups.length > 0 ? agentGroups[0].id : '', // Grupo por defecto
      avatar: '',
      whatsapp: ''
    });
    setFormErrors({});
  ...
  const validateForm = () => {
    ...
    // Validar que el grupo seleccionado esté dentro de los grupos del agente
    if (newUser.groupId && !agentGroups.some(group => group.id === parseInt(newUser.groupId))) {
      errors.groupId = "Solo puedes asignar usuarios a tus grupos asignados";
    }
  ...
  return (
    <Layout title="Gestión de Usuarios">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Administra los usuarios de tus grupos asignados</p>
            </div>
          </div>
        ...
        {/* Info sobre grupos asignados */}
        <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
          <div className="flex items-center space-x-2 mb-2">
            <SafeIcon icon={FiUsers} className="w-5 h-5 text-blue-600" />
            <h3 className="text-sm font-medium text-blue-800">Tus Grupos Asignados</h3>
          </div>
          <div className="flex flex-wrap gap-2">
            {agentGroups.map(group => (
              <span key={group.id} className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded-full">
                {group.name}
              </span>
            ))}
          </div>
          <p className="text-xs text-blue-600 mt-2">
            Solo puedes gestionar usuarios que pertenezcan a estos grupos
          </p>
        </div>
        ...
        {!showTrashCan && (
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-end">
              ...
              {/* Filtro por Grupo - Solo grupos del agente */}
              <div className="w-full md:w-48">
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Grupo
                </label>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className={`w-full border ${
                      formErrors.groupId ? 'border-red-300' : 'border-gray-300'
                    } rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary`}
                  >
