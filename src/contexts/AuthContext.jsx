...
export const AuthProvider=({children})=> {
  ...
  useEffect(()=> {
    ...
    if (savedUser) {
      try {
        ...
        setUser(userData);
        // Set userId for Quest Help System
        localStorage.setItem('userId', userData.id?.toString() || 'anonymous');
      } catch (error) {
  ...
  const login=async (email,password)=> {
    try {
      ...
      if (email.trim()==='superadmin@platform.com' && password.trim()==='superadmin123') {
        ...
        localStorage.setItem('userId', userData.id?.toString() || 'superadmin');
      ...
      // Admin credentials
        ...
        setUser(userData);
        localStorage.setItem('user',JSON.stringify(userData));
        localStorage.setItem('userId', userData.id?.toString() || 'admin');
        return {success: true};
      }
      // Staff credentials
        ...
        setUser(userData);
        localStorage.setItem('user',JSON.stringify(userData));
        localStorage.setItem('userId', userData.id?.toString() || 'staff');
        return {success: true};
      }
      // Client credentials
        ...
        setUser(userData);
        localStorage.setItem('user',JSON.stringify(userData));
        localStorage.setItem('userId', userData.id?.toString() || 'client');
        return {success: true};
      } else {
  ...
  const updateUserProfile=(updatedUserData)=> {
    ...
    localStorage.setItem('userId', updatedUserData.id?.toString() || 'anonymous');
  };
